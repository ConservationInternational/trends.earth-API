...
from gefapi.models import Execution, Script, User, StatusLog
...
class StatsService:
    ...
    def _get_from_cache_or_execute(cache_key: str, execution_func) -> Any:
        ...
        try:
            ...
            logger.error(f"Error executing stats function for cache key {cache_key}: {e}")
            raise

    @staticmethod
    def get_dashboard_stats(period: str = 'all', include: List[str] = None) -> Dict[str, Any]:
        """
        ...
        # Generate cache key
        cache_key = StatsService._get_cache_key('get_dashboard_stats', period=period, include=','.join(sorted(include)))
    ...
    @staticmethod
    def get_execution_stats(period: str = 'last_month', group_by: str = 'day', 
        ...
        def execute_stats():
            try:
                result = {
                    'time_series': StatsService._get_execution_time_series(period, group_by, task_type, status),
    ...
    @staticmethod
    def get_user_stats(period: str = 'last_year', group_by: str = 'month', 
        ...
        def execute_stats():
            try:
                result = {
                    'registration_trends': StatsService._get_registration_trends(period, group_by, country),
                    'geographic_distribution': StatsService._get_geographic_data(period),
    ...
    def _get_summary_stats() -> Dict[str, Any]:
        ...
        def execute_summary():
            ...
            for period_name, cutoff_date in periods.items():
                # Jobs count
                jobs_count = db.session.query(func.count(Execution.id)).filter(
                    Execution.start_date >= cutoff_date
                ).scalar() or 0
                summary[f'jobs_{period_name}'] = jobs_count
                
                # Users count (registered in period)
                users_count = db.session.query(func.count(User.id)).filter(
        ...
        def execute_trends():
            ...
            if period in ['last_day', 'all'] or cutoff_date is None:
                ...
                hourly_data = db.session.query(
                    ...
                    Execution.start_date >= hourly_cutoff
                ).group_by(
                    func.date_trunc('hour', Execution.start_date)
                ).order_by('hour').all()
            ...
            if cutoff_date:
                daily_data = db.session.query(
                    ...
                    func.date_trunc('day', Execution.start_date)
                ).order_by('date').all()
                
                trends['daily_jobs'] = [
                    {'date': row.date.date().isoformat() if row.date else None, 'count': row.count}
            ...
            monthly_data = db.session.query(
                ...
                Execution.start_date >= monthly_cutoff
            ).group_by(
                func.date_trunc('month', Execution.start_date)
            ).order_by('month').all()
    ...
    def _get_geographic_data(period: str) -> Dict[str, Any]:
        ...
        def execute_geographic():
            ...
            country_data = query.group_by(User.country).order_by(desc('user_count')).limit(20).all()
    ...
    def _get_task_stats(period: str) -> Dict[str, Any]:
        ...
        query = db.session.query(
            ...
            func.count(func.nullif(Execution.status != 'FINISHED', True)).label('success_count')
        ).join(
        ...
        by_version = [
            {
                'version': version,
                'count': count,
                'percentage': round((count / total_executions * 100), 1) if total_executions > 0 else 0
            }
    ...
    @staticmethod
    def _get_execution_time_series(period: str, group_by: str, task_type: Optional[str], 
        ...
        query = db.session.query(
            ...
            Script, Script.id == Execution.script_id
        )
    ...
    def _get_top_users(period: str, task_type: Optional[str]) -> List[Dict[str, Any]]:
        ...
        query = db.session.query(
            ...
            Execution, Execution.user_id == User.id
        ).join(
            Script, Script.id == Execution.script_id
        )
        ...
        for row in top_users_data:
            ...
            favorite_tasks = db.session.query(
                Script.slug,
                func.count(Execution.id).label('count')
            ).join(
                Execution, Execution.script_id == Script.id
            ).filter(
                Execution.user_id == row.id
            ).group_by(
                Script.slug
            ).order_by(
                desc('count')
            ...
            result.append({
                ...
                'favorite_tasks': [StatsService._normalize_task_name(task.slug) for task in favorite_tasks]
            })
    ...
    def _get_task_performance(period: str) -> List[Dict[str, Any]]:
        ...
        query = db.session.query(
            ...
            func.count(func.nullif(Execution.status != 'FINISHED', True)).label('success_count'),
            func.avg(
                func.extract('epoch', Execution.end_date - Execution.start_date) / 60
            ).label('avg_duration_minutes')
        ).join(
            Execution, Execution.script_id == Script.id
        ).filter(
            Execution.start_date.isnot(None),
    ...
    @staticmethod
    def _get_registration_trends(period: str, group_by: str, country: Optional[str]) -> List[Dict[str, Any]]:
        ...
        trends_data = query.group_by(
            func.date_trunc(trunc_format, User.created_at)
        ).order_by('date').all()
    ...
    def _get_activity_stats() -> Dict[str, Any]:
        ...
        active_last_day = db.session.query(func.count(func.distinct(Execution.user_id))).filter(
            Execution.start_date >= now - timedelta(days=1)
        ).scalar() or 0
        
        active_last_week = db.session.query(func.count(func.distinct(Execution.user_id))).filter(
            Execution.start_date >= now - timedelta(days=7)
        ).scalar() or 0
        
        active_last_month = db.session.query(func.count(func.distinct(Execution.user_id))).filter(
            Execution.start_date >= now - timedelta(days=30)
        ).scalar() or 0
        
        # Users who have never executed a job
        total_users = db.session.query(func.count(User.id)).scalar() or 0
        users_with_executions = db.session.query(func.count(func.distinct(Execution.user_id))).scalar() or 0
        ...
        try:
            ...
            if redis_cache.client:
                ...
                if keys:
                    ...
                    logger.info(f"Cleared {deleted_count} cache keys matching pattern: {cache_pattern}")
                    return True
                else:
            ...
            if redis_cache.client:
                ...
                for key in keys:
                    ...
                    cache_info['keys'].append({
                        'key': key,
                        'ttl_seconds': ttl,
                        'expires_in': f"{ttl // 60}m {ttl % 60}s" if ttl > 0 else "expired" if ttl == -1 else "no expiry"
                    })
