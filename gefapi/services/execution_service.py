...
<li>End time: {}</li> \
<li>Status: {}</li></ul> \
<p>For more information, and to view the results, open Trends.Earth and click the \"See Google Earth Engine tasks\" button.</p> \
<br>\
...
class ExecutionService(object):
    ...
    def get_executions(user, target_user_id=None, updated_at=None):
        ...
        if user.role == 'ADMIN':
            ...
            if target_user_id:
                try:
                    ...
                    raise error
                executions = db.session.query(Execution).filter(
                    Execution.user_id == target_user_id).filter(
        ...
        # ME
            ...
            executions = db.session.query(Execution).filter(
                Execution.user_id == user.id).filter(
                    Execution.end_date > updated_at).order_by(
                        Execution.end_date)
        ...
        if user == 'fromservice' or user.role == 'ADMIN':
            ...
            try:
                ...
                execution = db.session.query(Execution).filter(
                    Execution.id == execution_id).filter(
                        Execution.user_id == user.id).first()
            except Exception as error:
                raise error
    ...
    def create_execution_log(log, execution_id):
        ...
        if not execution:
            raise ExecutionNotFound(message='Execution with id ' +
                                    execution_id + ' does not exist')
        execution_log = ExecutionLog(text=text,
                                     level=level,
