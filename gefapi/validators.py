...
def validate_name(name):
    ...
    for char in clean_name:
        if not (unicodedata.category(char).startswith('L') or  # Letters
                unicodedata.category(char).startswith('M') or  # Marks (accents, etc.)
                char in " '-." or  # Allowed punctuation
                unicodedata.category(char) == 'Zs'):  # Spaces
...
def validate_country(country):
    ...
    for char in clean_country:
        if not (unicodedata.category(char).startswith('L') or  # Letters
                unicodedata.category(char).startswith('M') or  # Marks
                char in " '-." or  # Allowed punctuation
                unicodedata.category(char) == 'Zs'):  # Spaces
...
def validate_script_name(name):
    ...
    for char in clean_name:
        if not (unicodedata.category(char).startswith('L') or  # Letters
                unicodedata.category(char).startswith('M') or  # Marks
                unicodedata.category(char).startswith('N') or  # Numbers
                char in " -_." or  # Allowed punctuation
                unicodedata.category(char) == 'Zs'):  # Spaces
...
def validate_description(description, max_length=1000):
    ...
    clean_description = sanitize_text(description, max_length=max_length, allow_html=True)
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            if "institution" in json_data:
                json_data["institution"] = validate_institution(json_data["institution"])
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            if "description" in json_data:
                json_data["description"] = validate_description(json_data["description"])
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            if "status" in json_data:
                ...
                valid_statuses = ["SUBMITTED", "READY", "RUNNING", "FINISHED", "FAILED", "CANCELLED"]
            ...
            if "progress" in json_data:
                ...
                if not isinstance(progress, (int, float)) or progress < 0 or progress > 100:
                    return error(status=400, detail="Progress must be between 0 and 100")
...
def validate_execution_log_creation(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            json_data["text"] = sanitize_text(log_text, max_length=10000, allow_html=False)
