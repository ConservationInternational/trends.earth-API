...
def cleanup_docker_build_cache(self):
    ...
    try:
        ...
        # 1. Prune stopped containers FIRST (this unblocks image cleanup)
        # Dead/stopped containers hold references to images, preventing their removal
        try:
            logger.info("[TASK]: Pruning stopped containers...")
            ...
            containers_removed = len(containers_deleted)
            ...
            logger.warning(f"[TASK]: Failed to prune containers: {e}")

        # 2. Prune dangling images (untagged images from failed/incomplete builds)
        # Now more effective since container references are removed
        try:
        ...
        try:
            ...
            try:
                ...
                # Build cache prune failed - log but continue with other cleanup
                logger.warning(f"[TASK]: Failed to prune build cache: {inner_error}")
        ...
        # 3. Remove anonymous (unnamed) orphan volumes only.
        #    Named volumes (e.g. postgres data) MUST be preserved — a
        #    blanket ``volumes.prune()`` would destroy them if the owning
        #    container happens to be stopped during a deploy window.
        try:
            logger.info("[TASK]: Removing anonymous orphan volumes...")
            all_volumes = docker_client.volumes.list()
            anonymous_volumes = [
                v
                for v in all_volumes
                # Anonymous volumes have a 64-char hex name generated by
                # Docker; named volumes have a human-readable name.
                if len(v.name) == 64 and all(c in "0123456789abcdef" for c in v.name)
            ]
            ...
            logger.info(
                "[TASK]: Removed %d anonymous orphan volumes",
                volumes_removed,
            )
        except Exception as e:
            logger.warning(f"[TASK]: Failed to clean up volumes: {e}")
        ...
        result = {
            ...
            "containers_pruned": containers_removed,
        ...
        logger.info(
            "[TASK]: Docker resource cleanup complete. "
            "Removed %d containers, %d images, reclaimed %s",
            containers_removed,
            images_removed,
            space_str,
        )
