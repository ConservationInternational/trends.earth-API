...
class StagingEnvironmentSetup:
    def __init__(self):
        ...
        self.test_users = [
            {
                "email": os.getenv("TEST_SUPERADMIN_EMAIL", "test-superadmin@example.com"),
        ...
        required_vars = ["TEST_SUPERADMIN_PASSWORD", "TEST_ADMIN_PASSWORD", "TEST_USER_PASSWORD"]
    ...
    def create_test_users(self):
        ...
        try:
            ...
            for user_data in self.test_users:
                ...
                logger.info(f"✓ User {user_data['email']} created/updated with ID: {actual_user_id}")
    ...
    def copy_recent_scripts(self, superadmin_id):
        ...
        if not self.prod_db_config["password"]:
            logger.warning("No production database password provided, skipping script import")
            return
        ...
        if not prod_conn:
            logger.warning("Could not connect to production database, skipping script import")
            return
        ...
        try:
            ...
            prod_cursor.execute("""
                SELECT id, name, slug, description, created_at, updated_at, status, public, 
            ...
            for script in scripts:
                try:
                    ...
                    staging_cursor.execute("""
                        INSERT INTO script (id, name, slug, description, created_at, updated_at, 
                    ...
                    """, (
                        ...
                        superadmin_id, script[6], script[7], script[8], script[9],
                        script[10], script[11], script[12], script[13]
                    ))
    ...
    def copy_recent_status_logs(self):
        ...
        if not self.prod_db_config["password"]:
            logger.warning("No production database password provided, skipping status logs import")
            return
        ...
        prod_conn = self.connect_to_database(self.prod_db_config)
        if not prod_conn:
            logger.warning("Could not connect to production database, skipping status logs import")
            return
        ...
        try:
            ...
            prod_cursor.execute("""
                SELECT id, timestamp, executions_active, executions_ready, executions_running,
                       executions_finished, executions_failed, executions_count, users_count,
                       scripts_count, memory_available_percent, cpu_usage_percent
                FROM status_log 
                WHERE timestamp >= %s
            ...
            for log in status_logs:
                try:
                    staging_cursor.execute("""
                        INSERT INTO status_log (id, timestamp, executions_active, executions_ready,
    ...
    def copy_script_logs(self):
        ...
        if not self.prod_db_config["password"]:
            logger.warning("No production database password provided, skipping script logs import")
            return
            
        logger.info("Copying script logs for imported scripts from production database...")
        
        prod_conn = self.connect_to_database(self.prod_db_config)
        if not prod_conn:
            logger.warning("Could not connect to production database, skipping script logs import")
            return
        ...
        try:
            ...
            for log in script_logs:
                try:
                    ...
                    staging_cursor.execute("SELECT 1 FROM script WHERE id = %s", (log[3],))
    ...
    def verify_setup(self):
        ...
        try:
            ...
            for user_data in self.test_users:
                logger.info(f"  {user_data['role']}: {user_data['email']} (password: {user_data['password']})")
