...
class TestDockerCancellationTasks:
    ...
    @patch('gefapi.services.docker_service.get_docker_client')
    def test_cancel_execution_task_both_service_and_container(self, mock_get_docker_client):
    ...
    @patch('gefapi.services.docker_service.get_docker_client')
    def test_cancel_execution_task_multiple_matching_services(self, mock_get_docker_client):
    ...
    def test_cancel_execution_task_partial_failure(self, mock_get_docker_client):
        ...
        assert result["docker_service_stopped"] is True
        assert result["docker_container_stopped"] is False
        assert len(result["errors"]) == 1  # Only one error since container operations are in try-except block
        assert any("Docker container stop failed" in error for error in result["errors"])
    ...
    def test_cancel_execution_task_logging(self, mock_logger, mock_get_docker_client):
        ...
        assert any("Celery task: Canceling Docker resources for execution" in msg for msg in log_messages)
        assert any("Stopping Docker service" in msg for msg in log_messages)

    @patch('gefapi.services.docker_service.get_docker_client')
    def test_cancel_execution_task_container_name_variations(self, mock_get_docker_client):
        ...
        mock_container2.name = "execution-test-execution-id-something"  # Should not match
...
class TestDockerServiceTaskIntegration:
    ...
    def test_execution_service_calls_docker_task_correctly(self, mock_celery_app, mock_get_docker_client):
        ...
        with (
            ...
            patch("gefapi.services.gee_service.GEEService.cancel_gee_tasks_from_execution") as mock_gee_cancel,
