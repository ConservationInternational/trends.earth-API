...
from gefapi.utils.rate_limiting import create_rate_limit_response, rate_limit_error_handler
...
class TestRateLimitingRollbarNotifications:
    ...
    def test_rollbar_notification_with_authenticated_user(
        self, mock_get_remote_address, mock_verify_jwt, mock_get_current_user, mock_rollbar, client
    ):
        ...
        with client.application.test_request_context(
            ...
            # Check the message
            assert "Rate limit applied to user test@example.com (ID: 123)" in call_args[1]["message"]
    ...
    def test_rollbar_notification_with_unauthenticated_user(
        ...
        with client.application.test_request_context(
            '/auth', 
            method='POST',
            headers={'User-Agent': 'Auth Client'}
        ):
            # Call the rate limit response function
    ...
    def test_rate_limit_error_handler_calls_create_response(self, mock_create_response, client):
    ...
    @patch('gefapi.utils.rate_limiting.create_rate_limit_response')
    def test_rate_limit_error_handler_handles_missing_retry_after(self, mock_create_response, client):
