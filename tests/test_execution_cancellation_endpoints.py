...
class TestExecutionCancellationEndpoints:
    ...
    def test_cancel_execution_success_own_execution(self, client, auth_headers_user, regular_user, sample_script, db_session):
        ...
        with patch('gefapi.services.execution_service.celery_app') as mock_celery:
            with patch.object(mock_celery, 'send_task') as mock_send_task:
                ...
                # Mock GEE service
                with patch('gefapi.services.gee_service.GEEService.cancel_gee_tasks_from_execution') as mock_gee:
    ...
    def test_cancel_execution_admin_cancel_other_user(self, client, auth_headers_admin, regular_user, sample_script, db_session):
    ...
    def test_cancel_execution_forbidden_other_user(self, client, auth_headers_user, admin_user, sample_script, db_session):
        ...
        assert response.status_code in [403, 404]  # Both are valid - depends on implementation
        response_data = response.get_json()
        assert "detail" in response_data
        # Either "forbidden" or "not found" message is acceptable
        assert any(keyword in response_data["detail"].lower() for keyword in ["forbidden", "not found", "only cancel your own"])
    ...
    def test_cancel_execution_not_found(self, client, auth_headers_user):
        ...
        response = client.post(
            f"/api/v1/execution/{non_existent_id}/cancel",
            headers=auth_headers_user
        )
    ...
    def test_cancel_execution_already_finished(self, client, auth_headers_user, regular_user, sample_script, db_session):
    ...
    def test_cancel_execution_already_cancelled(self, client, auth_headers_user, regular_user, sample_script, db_session):
    ...
    def test_cancel_execution_failed_status(self, client, auth_headers_user, regular_user, sample_script, db_session):
    ...
    def test_cancel_execution_unauthorized(self, client, regular_user, sample_script, db_session):
        ...
        assert response.status_code in [401, 422]  # JWT required error - could be either

    def test_cancel_execution_docker_task_timeout(self, client, auth_headers_user, regular_user, sample_script, db_session):
        ...
        with patch('gefapi.services.execution_service.celery_app') as mock_celery:
            with patch.object(mock_celery, 'send_task') as mock_send_task:
                ...
                mock_task_result = Mock()
                mock_task_result.get.side_effect = Exception("Worker lost")  # Generic exception instead of WorkerLostError
                mock_send_task.return_value = mock_task_result
    ...
    def test_cancel_execution_with_gee_tasks(self, client, auth_headers_user, regular_user, sample_script, db_session):
    ...
    def test_cancel_execution_service_error_recovery(self, client, auth_headers_user, regular_user, sample_script, db_session):
        ...
        with patch('gefapi.services.execution_service.celery_app') as mock_celery:
            with patch.object(mock_celery, 'send_task') as mock_send_task:
                ...
                mock_task_result.get.return_value = {
                    "docker_service_stopped": False,
                    "docker_container_stopped": False,
                    "errors": ["Docker service not found", "Docker container not found"]
                }
                mock_send_task.return_value = mock_task_result
    ...
    def test_cancel_execution_invalid_uuid(self, client, auth_headers_user):
        ...
        response = client.post(
            "/api/v1/execution/invalid-uuid/cancel",
            headers=auth_headers_user
        )

        # Should return error for invalid UUID
        assert response.status_code in [400, 404, 500]  # Depends on validation implementation

    def test_cancel_execution_logs_created(self, client, auth_headers_user, regular_user, sample_script, db_session):
        ...
        with patch('gefapi.services.execution_service.celery_app') as mock_celery:
            with patch.object(mock_celery, 'send_task') as mock_send_task:
                ...
                initial_log_count = ExecutionLog.query.filter_by(execution_id=execution.id).count()
                ...
                with patch('gefapi.services.gee_service.GEEService.cancel_gee_tasks_from_execution') as mock_gee:
                    ...
                    assert any(phrase in log_text for phrase in ["GEE task", "1/1", "cancelled"])
...
class TestExecutionCancellationIntegration:
    ...
    def test_full_cancellation_workflow(self, client, auth_headers_user, regular_user, sample_script, db_session):
