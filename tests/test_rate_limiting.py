...
class TestRateLimitReset:
    ...
    def test_environment_user_exemption_from_rate_limiting(
        self, client, environment_user_token
    ):
        """Test that the automation user is exempt from rate limiting regardless of role"""
        auth_headers = {"Authorization": f"Bearer {environment_user_token}"}

        # Make many requests as the automation user - should not be rate limited
        automation_responses = []
        for i in range(50):  # Make many requests
            response = client.get("/api/v1/user/me", headers=auth_headers)
            automation_responses.append(response)

            # If we get rate limited, this test should fail
            if response.status_code == 429:
                break

        # Automation user should never get rate limited (even though role is USER)
        rate_limited_responses = [
            r for r in automation_responses if r.status_code == 429
        ]
        assert len(rate_limited_responses) == 0, (
            "Environment automation user should be exempt from rate limiting regardless of role"
        )

        # Most responses should be successful
        successful_responses = [
            r for r in automation_responses if r.status_code in [200, 403]
        ]
        assert len(successful_responses) > 40, (
