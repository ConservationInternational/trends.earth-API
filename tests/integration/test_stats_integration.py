...
from tests.conftest import assert_api_response_structure, assert_error_response_structure
...
class TestStatsIntegration:
    ...
    def test_complete_dashboard_stats_flow(self, mock_is_superadmin, client, auth_headers_superadmin, mock_redis):
        ...
        with patch('gefapi.services.stats_service.StatsService._get_summary_stats') as mock_summary:
            ...
            response2 = client.get('/api/v1/stats/dashboard', headers=auth_headers_superadmin)
    ...
    def test_cache_management_integration(self, mock_is_superadmin, client, auth_headers_superadmin, mock_redis):
    ...
    @patch('gefapi.routes.api.v1.stats.is_superadmin')
    def test_error_handling_integration(self, mock_is_superadmin, client, auth_headers_superadmin, mock_redis):
        ...
        with patch('gefapi.services.stats_service.StatsService._get_summary_stats') as mock_summary, \
             patch('gefapi.services.stats_service.StatsService._get_trends_data') as mock_trends, \
             patch('gefapi.services.stats_service.StatsService._get_geographic_data') as mock_geo, \
             patch('gefapi.services.stats_service.StatsService._get_task_stats') as mock_tasks:
    ...
    @patch('gefapi.utils.permissions.is_superadmin')
    def test_parameter_validation_integration(self, mock_is_superadmin, client, auth_headers_superadmin):
        """Test parameter validation through the API."""
        mock_is_superadmin.return_value = True
        
        # Test invalid period
        response = client.get('/api/v1/stats/dashboard?period=invalid', headers=auth_headers_superadmin)
        assert response.status_code == 400
        data = json.loads(response.data)
        assert_error_response_structure(data)
        
        # Test invalid include sections
        response = client.get('/api/v1/stats/dashboard?include=invalid,summary', headers=auth_headers_superadmin)
        assert response.status_code == 400
        data = json.loads(response.data)
        assert_error_response_structure(data)
        
        # Test invalid execution status
        response = client.get('/api/v1/stats/executions?status=INVALID', headers=auth_headers_superadmin)
        assert response.status_code == 400
        data = json.loads(response.data)
        assert_error_response_structure(data)
    ...
    @patch('gefapi.routes.api.v1.stats.is_superadmin')
    def test_performance_integration(self, mock_is_superadmin, client, auth_headers_superadmin, mock_redis):
    ...
    @patch('gefapi.utils.permissions.is_superadmin')
    def test_data_consistency_integration(self, mock_is_superadmin, client, auth_headers_superadmin, mock_redis):
        ...
        with patch('gefapi.services.stats_service.StatsService._get_summary_stats') as mock_summary, \
             patch('gefapi.services.stats_service.StatsService.get_execution_stats') as mock_exec_stats, \
             patch('gefapi.services.stats_service.StatsService.get_user_stats') as mock_user_stats:
            ...
            # Get dashboard stats
            dashboard_response = client.get('/api/v1/stats/dashboard', headers=auth_headers_superadmin)
            assert dashboard_response.status_code == 200
            dashboard_data = json.loads(dashboard_response.data)
            
            # Get execution stats
            execution_response = client.get('/api/v1/stats/executions', headers=auth_headers_superadmin)
            assert execution_response.status_code == 200
            execution_data = json.loads(execution_response.data)
            
            # Get user stats
            user_response = client.get('/api/v1/stats/users', headers=auth_headers_superadmin)
    ...
    @patch('gefapi.utils.permissions.is_superadmin')
    def test_health_check_integration(self, mock_is_superadmin, client, auth_headers_superadmin):
    ...
    @patch('gefapi.utils.permissions.is_superadmin')
    def test_concurrent_access_integration(self, mock_is_superadmin, client, auth_headers_superadmin, mock_redis):
