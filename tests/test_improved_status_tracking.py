...
from gefapi.services.execution_service import ExecutionService, update_execution_status_with_logging
...
class TestImprovedStatusTracking:
    ...
    def test_status_log_model_new_schema(self, app):
        ...
        with app.app_context():
            ...
            expected_fields = {
                "id", "timestamp", "executions_active", "executions_ready",
    ...
    def test_update_execution_status_with_logging_creates_status_log(self, app):
        ...
        with app.app_context():
            ...
            # Verify a new status log was created
    ...
    def test_update_execution_status_with_logging_counts_executions(self, app):
        ...
        with app.app_context():
            ...
            executions = []
            statuses = ["PENDING", "READY", "RUNNING", "FINISHED", "FAILED", "CANCELLED"]
            ...
            # Verify counts in status log
            assert status_log.executions_ready == 1  # Only the original READY
            assert status_log.executions_running == 2  # RUNNING + PENDING (original) + new RUNNING
            ...
            assert status_log.executions_active == 3  # ready + running (1 + 2)
    ...
    def test_helper_function_handles_terminal_states(self, app):
        ...
        with app.app_context():
            ...
            for terminal_status in ["FINISHED", "FAILED", "CANCELLED"]:
                ...
                status_log = update_execution_status_with_logging(execution, terminal_status)
